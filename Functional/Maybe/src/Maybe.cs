...
/// <summary>Composable version of <see cref="Nullable{T}"/> for value types and non-nullable reference types.</summary>
/// <remarks>
///     <para>
///         The values can be in one of two main states; Some or None.
///         <dl>
///             <dt>Some</dt>
///             <dd>Analogous to a <see cref="Nullable{T}"/> or nullable reference with a value.</dd>
///             <dt>None</dt>
///             <dd>
///                 Analogous to a <see cref="Nullable{T}"/> or nullable reference without a value, i.e. the infamous
///                 <see langword="null"/>.
///             </dd>
///         </dl>
///     </para>
...
///         <see
///             href="https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/patterns#list-patterns">
///             List Patterns
///         </see>.
/// <code>
...
///         <see cref="Maybe{T}"/> and overrides <see cref="object.Equals(object?)"/>. The comparison works in the same
///         way as equality comparison in <see cref="Nullable{T}"/>.
///     </para>
///     <para>
///         <see cref="Maybe{T}"/> implements <see cref="IFormattable"/> which implementation <see cref="ToString()"/>
///         and <see cref="ToString(string?)"/> delegates to.
///     </para>
///     <para>
///         <see cref="Maybe{T}"/> implements <see cref="IEnumerable{T}"/>. The enumerator will yield zero or one
///         element for none or some respectively. This enables <see cref="Maybe{T}"/> to be functionally bound
///         (flattened) together with any <see cref="IEnumerable{T}"/>.
/// <code>
...
///         <see cref="IList{T}.this">indexer</see> have an implicit implementation to support C# 11 list pattern. (It
///         also has an explicit implementation because of different exception semantics between <see cref="Maybe{T}"/>
///         and <see cref="IList{T}"/>.)
///     </para>
///     <para>
...
///         chaining of <see cref="Maybe{T}"/> values in logical expressions. Using the conditional operators enables
///         on-demand evaluation as expected.
///     </para>
///     <para>
///         <see cref="Maybe{T}"/> implements implicit cast operator from <typeparamref name="T"/>. Thus, returning some
///         value from a <see cref="Maybe{T}"/>-returning function is no effort. Returning none from a
///         <see cref="Maybe{T}"/>-returning function is also simple as it is the default state and from C# 12 also
///         supports empty collection initializer syntax. <c>return Maybe.None&lt;T&gt;();</c>, <c>return default;</c>
///         and <c>return [];</c> are equivalent.
///     </para>
/// </remarks>
...
public readonly struct Maybe<T> : IEquatable<Maybe<T>>, IFormattable, IList<T> where T : notnull
{
    ...
    /// <summary>Gets the number of elements contained in the <see cref="Maybe{T}"/></summary>
    /// <remarks>
    ///     <para>Provided for support of List Pattern of C# 11.</para>
    ///     <para>Use this property directly if you like, but it'll look rather silly.</para>
    /// </remarks>
    ...
    ///         appropriate.
    ///         <see href="https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/ca2201">
    ///             CA2201
    ///         </see>
    ///         rules this out as that exception is reserved for the CLR.
    ///     </para>
    ///     <para>
    ///         One could argue for throwing <see cref="ArgumentOutOfRangeException"/> which the <see cref="IList{T}"/>
    ///         <see cref="IList{T}.this">indexer</see>
    ///         <see href="https://learn.microsoft.com/en-us/dotnet/api/system.collections.ilist.item?view=net-8.0">
    ///             throw
    ///         </see>.
    ...
    ///         <see cref="Nullable{T}.Value"/> throw. <see cref="Maybe{T}"/> is a better <see cref="Nullable{T}"/>.
    ///         That will do.
    ///     </para>
    /// </remarks>
    /// <returns>Value if this is some and <paramref name="index"/> is 0.</returns>
    /// <exception cref="InvalidOperationException">
    ///     Thrown if this is none or <paramref name="index"/> is not 0. I trust the C# compiler never to trigger this.
    /// </exception>
    ...
    /// <returns>
    ///     <see langword="true"/> if values have the same state and if some, the same value; otherwise
    ///     <see langword="false"/>.
    /// </returns>
    /// <remarks>
    ///     If <typeparamref name="T"/> implements <see cref="IEquatable{T}"/>, that implementation is used to compare
    ///     some values; otherwise, reference equality or default value type equality is used to compare some values.
    /// </remarks>
    [PublicAPI]
    ...
    /// <summary>Produces an <see cref="IEnumerator{T}"/> over this <see cref="Maybe{T}"/>.</summary>
    /// <returns>An enumerator yielding the value in some case; otherwise not yielding any value.</returns>
    [PublicAPI]
    [Obsolete(
    ...
    ///         value if it is <see cref="IFormattable"/>; otherwise <see cref="object.ToString()"/> is used to produce
    ///         the value representation.
    ///     </para>
    ///     <para>Some is represented as "[&lt;value&gt;]". None is represented as "[]".</para>
    /// </remarks>
    /// <inheritdoc/>
    [PublicAPI]
    ...
    /// <summary>Produces the string representation of the value.</summary>
    /// <remarks>
    ///     <para><see cref="object.ToString()"/> is used to produce the value representation.</para>
    ///     <para>Some is represented as "[&lt;value&gt;]". None is represented as "[]".</para>
    /// </remarks>
    [PublicAPI]
    ...
    /// <remarks>
    ///     <para>
    ///         <paramref name="format"/> will be passed to
    ///         <see cref="IFormattable.ToString(string?, IFormatProvider?)"/> on the value if it is
    ///         <see cref="IFormattable"/>; otherwise <see cref="object.ToString()"/> is used to produce the value
    ///         representation.
    ///     </para>
    ///     <para>Some is represented as "[&lt;value&gt;]". None is represented as "[]".</para>
    /// </remarks>
    ...
    /// <summary>Truth value of <paramref name="value"/>.</summary>
    /// <remarks>
    ///     <para>
    ///         This operator is used when a <see cref="Maybe{T}"/> value is used in a controlling conditional
    ///         expression in e.g. <see langword="if"/>, <see langword="do"/>, <see langword="while"/> and
    ///         <see langword="for"/> statements and the ternary conditional operator (<see langword="?:"/>).
    ///     </para>
    ///     <para>
    ...
    /// <summary>Falsity value of <paramref name="value"/>.</summary>
    /// <remarks>
    ...
    /// <summary>Logical conjunction of operands <paramref name="left"/> and <paramref name="right"/>.</summary>
    /// <remarks>
    ...
    /// <summary>Logical disjunction of operands <paramref name="left"/> and <paramref name="right"/>.</summary>
    /// <remarks>
    ///     <c>maybeX | maybeY | maybeZ</c> evaluates to the leftmost operand which is some; otherwise none.
    /// </remarks>
    ...
    ///     </para>
    ///     <para>
    ///         <c>maybeX | maybeY | z</c> evaluates to the value of the leftmost operand which is some; otherwise
    ///         <c>z</c>.
    ///     </para>
    /// </remarks>
    ...
    /// <remarks>
    ///     <para>
    ///         This operator provides lazy fallback value much like <c>maybeX || z</c> would if C# could handle
    ///         conditional logical disjunction of different types.
    ///     </para>
    ///     <para>
    ///         <c>maybeX | getZ</c> evaluates to the value of <c>maybeX</c> if it is some; otherwise the return value
    ///         of <c>getZ</c>.
    ///     </para>
    /// </remarks>
    /// <returns>
    ...
    /// <summary>Equality operator of operands <paramref name="left"/> and <paramref name="right"/>.</summary>
    /// <returns>
    ///     <see langword="true"/> if operands have same state and in some case, the same value; otherwise
    ///     <see langword="false"/>.
    /// </returns>
    [PublicAPI]
    public static bool operator ==(Maybe<T> left, Maybe<T> right) => left.Equals(right);

    /// <summary>Inequality operator of operands <paramref name="left"/> and <paramref name="right"/>.</summary>
    /// <returns>
    ///     <see langword="true"/> if operands have different state or in some case, different value; otherwise
    ///     <see langword="false"/>.
    /// </returns>
    [PublicAPI]
    public static bool operator !=(Maybe<T> left, Maybe<T> right) => !left.Equals(right);

    /// <summary>Implicit cast from <typeparamref name="T"/> to <see cref="Maybe{T}"/>.</summary>
    [PublicAPI]
...
/// <summary>Factory and extension methods for <see cref="Maybe{T}"/>.</summary>
/// <remarks>A couple of useful extensions for <see cref="IEnumerable{T}"/> are also provided.</remarks>
public static class Maybe
{
    ...
    /// <summary>
    ///     Sequential application of <paramref name="maybeFunc"/> to <paramref name="maybeArg"/> in applicative functor
    ///     of <see cref="Maybe{T}"/>.
    /// </summary>
    ...
    /// <remarks>This is the equivalent of the infix operator <see langword="&gt;&gt;="/> of Monad in Haskell.</remarks>
    /// <returns>Lifted return value.</returns>
    [PublicAPI]
    ...
    /// <summary>Filters a <see cref="Maybe{T}"/> value based on a predicate function.</summary>
    ...
    ///     The type of the intermediate value produced by <paramref name="selector"/>. In the documentation of
    ...
    ///     </para>
    ///     <para>
    ///         The difference between <see cref="Bind"/> and <see cref="SelectMany"/> is that <see cref="SelectMany"/>
    ///         takes a binary projection function, <paramref name="resultSelector"/>, as a parameter and as such can
    ///         chain calls to <see cref="SelectMany"/> instead of encapsulating calls to <see cref="Bind"/> in nested
    ///         closures.
    ///     </para>
    ///     <para>
    ///         <see cref="SelectMany"/> can be implemented in terms of <see cref="Bind"/> and <see cref="Map"/> but
    ///         type-specific implementations are probably more efficient.
    ///     </para>
    /// </remarks>
    [PublicAPI]
    ...
    /// <summary>Traverse <see cref="Maybe{T}"/> with an asynchronous function.</summary>
    /// <typeparam name="TSource">Type of source.</typeparam>
    /// <typeparam name="TResult">Type of result.</typeparam>
    /// <param name="source">Maybe value to traverse.</param>
    /// <param name="func">Asynchronous function to traverse with.</param>
    /// <returns>
    ...
    /// <summary>Code style extension to use Try-style method syntax with a value of <see cref="Maybe{T}"/>.</summary>
    /// <typeparam name="TSource">Type of value.</typeparam>
    /// <param name="source">Maybe value.</param>
    /// <param name="value">Inner value in some case; otherwise undefined (<see langword="default"/>! really).</param>
    /// <returns><see langword="true"/> in some case; otherwise <see langword="false"/>.</returns>
    /// <remarks>
    ///     <para>
    ...
    /// <summary>Filters a <see cref="Maybe{T}"/> value based on a predicate.</summary>
    /// <typeparam name="TSource">The type of the value of <paramref name="source"/>.</typeparam>
    /// <param name="source">A <see cref="Maybe{T}"/> to filter.</param>
    /// <param name="predicate">A function to test some value for a condition.</param>
    /// <returns>
    ...
    /// <summary>Conversion from <see cref="Nullable{T}"/> to <see cref="Maybe{T}"/>.</summary>
    ...
    /// <summary>Conversion from nullable reference type <typeparamref name="T"/>? to <see cref="Maybe{T}"/>.</summary>
    /// <typeparam name="T">Type of some value.</typeparam>
    ...
    /// <summary>
    ///     Returns some only element of a sequence that satisfies a specified condition or none if no such element
    ...
    /// <summary>Returns some first element of a sequence, or none if the sequence contains no elements.</summary>
    /// <typeparam name="T">The type of the elements of <paramref name="source"/>.</typeparam>
    ...
    /// <param name="predicate">A function to test an element for a condition.</param>
    /// <returns>
    ///     Some first element in <paramref name="source"/> that passes the test specified by
    ///     <paramref name="predicate"/> if such an element is found; otherwise none.
    /// </returns>
    [PublicAPI]
