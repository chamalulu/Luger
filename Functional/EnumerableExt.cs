...
namespace Luger.Functional
{
    ...
    public static class EnumerableExt
    {
        ...
        public static IEnumerable<(T value, int index)> WithIndex<T>(this IEnumerable<T> ts)
            => ts.Select((t, i) => (t, i));

        public static IEnumerable<T> EveryNth<T>(this IEnumerable<T> ts, int n)
        {
            if (n <= 0)
                throw new ArgumentOutOfRangeException(nameof(n));

            return
                from t in ts.WithIndex()
                where t.index % n == 0
                select t.value;
        }
