...
namespace Luger.Matrices
{
    public class Matrix
    {
        ...
        protected abstract class Vector : IEnumerable<double>
        {
            ...
            public static double operator *(Vector u, Vector v)
            {
                if (u.Dimension != v.Dimension)
                    throw new ArithmeticException("Dimensions mismatch in dot product vector multiplication.");
        ...
        // Naive matrix multiplication implementation. You need serious implementations for better performance.
        public static Matrix operator *(Matrix left, Matrix right)
        {
            if (left.Columns.Count != right.Rows.Count)
                throw new ArithmeticException("Inner dimensions mismatch in matrix multiplication.");
